...
def confirmOrder(request,orderid):
    ...
    if request.method=='POST':
        ...
        if promoExists:
            ...
            #x=Order.objects.get(userAccount=orderObj.userAccount)#.update(adjustedSum=str(adjustedCost))                                                                            #x.adjustedSum=str(adjustedCost)                                                                                                                                                 
...
def checkout(request,orderid):
    ...
    if request.method=='POST': #if submit is clicked
        form=paymentForm(initial={'userAccount': request.user})
        form=paymentForm(request.POST)
        if form.is_valid():
            print('curruser',curruser)
            print('curruser id',curruser.id)
            form.save(commit=False)
            print('form.useraccount after saving: ',form.userAccount)
            context={
                'title':title,
                'total':adjustedSum,
        ...
        if paymentInfo:
            form=paymentForm(instance=paymentInfo)
        else:
            form=paymentForm()
            context={
                'title':title,
